...
@dataclass
class DataCollatorForSupervisedAlignmentDataset:
    """Collate examples for supervised alignment fine-tuning.
    This class is originally implemented by the LLaVA team and
    modified by Haotian Tang. Further, this class is modified by Luozheng Qin from SAIS to support alignment training"""

    tokenizer: transformers.PreTrainedTokenizer
    siglip_processor: transformers.SiglipImageProcessor
    data_args: DataArguments
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # to truncated <image> tokens later.
        for _images, _input_ids in zip(images, input_ids):
            if len(_images) != (_input_ids == IMAGE_TOKEN_INDEX).sum().item():
                print(instances)
            assert (
                len(_images) == (_input_ids == IMAGE_TOKEN_INDEX).sum().item()
            ), f"Number mismatch between images and placeholder image tokens in 'len(_images) == (_input_ids == IMAGE_TOKEN_INDEX).sum().item()'.\
        ...
        # @qinluozheng: support images to be aligned
        if not any(["tgt_img" not in instance.keys() for instance in instances]):
            batch["aligned_images"] = [self.siglip_processor(images=[instance["tgt_img"]], return_tensors="pt") for instance in instances]
            # @qinluozheng: for alignment debug
            batch["debug_tgt_image"] = [instance["debug_tgt_image"] for instance in instances]
            batch["debug_prompt"] = [instance["debug_prompt"] for instance in instances]
            if not any(["debug_src_image" not in instance.keys() for instance in instances]):
                batch["debug_src_image"] = [instance["debug_src_image"] for instance in instances]
        return batch
...
def make_supervised_data_module(
    ...
    data_args: DataArguments,
    training_args: TrainingArguments,
    for_alignment=False # @qinluozheng: our unified_gen model need alignment training first, which requires a specialized data collator.
) -> Dict:
    ...
    if PROCESS_GROUP_MANAGER is None:
        if not for_alignment:
            ...
            siglip_processor = transformers.SiglipImageProcessor.from_pretrained(training_args.aligned_siglip_model_path)
            data_collator = DataCollatorForSupervisedAlignmentDataset(
                                tokenizer=tokenizer, 
...
def build_datasets(
    ...
    for dataset in mixture:
        ...
        elif dataset_type == "unified_gen":
            dataset_cls = LazyDiffSupervisedDataset
            if hasattr(dataset, "image_path"):
                image_folder = dataset.image_path            
...
class LazyDiffSupervisedDataset(Dataset):
    """Dataset for supervised fine-tuning.
    This class is originally implemented by the LLaVA team and modified by
    Ji Lin and Haotian Tang. Further, Luozheng Qin from SAIS modified this class to support unified generation.
    """
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME # @qinluozheng: an issue originated from VILA project
        if "image" in sources[0]:
            image_file = self.list_data_dict[i]["image"]
            if isinstance(image_file, list):
                image = torch.stack([process_image(img, self.data_args, self.image_folder) for img in image_file])
                ...
                image = process_image(image_file, self.data_args, self.image_folder)
            sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
        elif "images" in sources[0]: # @qinluozheng: it is close to idneticial to use image or images, except the latter can exclusively process a list of dicts 
            all_images = []
            for image_file in self.list_data_dict[i]["images"]:
                if isinstance(image_file, dict):
                    image_file = image_file["path"]
        ...
        # unified generation image target
        if "tgt_image" in self.list_data_dict[i]:
            tgt_img = PIL.Image.open(self.list_data_dict[i]["tgt_image"]).convert("RGB")
            data_dict["tgt_img"] = tgt_img

        # for debug
        data_dict["debug_prompt"] = self.list_data_dict[i]["conversations"][0]["value"]
        if "image" in self.list_data_dict[i]:
            data_dict["debug_src_image"] = self.list_data_dict[i]["image"]
        data_dict["debug_tgt_image"] = self.list_data_dict[i]["tgt_image"]
