...
from llava.train.llava_trainer import VILADPOTrainer # @qinluozheng: DPO is not supported
...
from llava.train.vila_with_vh_trainer import VILAWithVisionHeadTrainer
...
def train():
    ...
    if resume_path:
        ...
        if training_args.lora_enable:
            ...
            config = LlavaLlamaConfig.from_pretrained(model_args.model_name_or_path, resume=resume_from_checkpoint)
            config.resume_path = model_args.model_name_or_path
        config.vision_head_resume_ckpt = resume_path
    else:
        ## first time training
    ...
    # @qinluozheng: wrap vila vlm into a vila_vh model

    #import pdb; pdb.set_trace()
    model = VILAWithVisionHeadForAlignment(
                vila=model,
                siglip=training_args.aligned_siglip_model_path,
                llm_hidden_size=4096,
            )
    if True: # set learnable parameters for alignment training with vila_with_vh
        for n, p in model.named_parameters():
            if "lm_head" in n:
                p.requires_grad = True
            elif "vision_head" in n:
    ...
    if training_args.dpo:
        ...
        trainer = VILAWithVisionHeadTrainer(model=model, tokenizer=tokenizer, args=training_args, callbacks=callbacks, **data_module)
    print(
    ...
    model.vlm.llm.config.use_cache = True
