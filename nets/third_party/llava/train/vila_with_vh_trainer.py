...
class VILAWithVisionHeadTrainer(Trainer):
    ...
    def log(self, logs: Dict[str, float]) -> None:
        ...
        for k in logs.keys(): # the implementation is not so elegant
            self.additional_logs[k] = logs[k]

        self.control = self.callback_handler.on_log(self.args, self.state, self.control,  self.additional_logs)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        """
        # implement the loss
        if "aligned_images" in inputs:
            aligned_images = inputs.pop("aligned_images")
            # test_tgt_img = inputs.pop("test_tgt_img")
            # print(test_tgt_img)
            # import pdb; pdb.set_trace()
            aligned_images = [aligned_image["pixel_values"] for aligned_image in aligned_images]
            aligned_images = torch.cat(aligned_images, dim=0)
        else:
            aligned_images = None
        # debug must be run with batch size = 1 and n_gpu = 1
        debug_prompt = inputs.pop("debug_prompt")[0]
        if "debug_src_image" in inputs:
            debug_src_image = inputs.pop("debug_src_image")[0]
        debug_tgt_image = inputs.pop("debug_tgt_image")[0]
        vila_vh_outputs = self.model(inputs, aligned_images)
        loss = vila_vh_outputs.pop("loss")
        outputs = vila_vh_outputs.pop("vlm_outputs")
        siglip2_img_pooled_output = vila_vh_outputs.pop("siglip2_img_pooled_output")

        if False:
            debugOutputDir = "/cpfs01/projects-HDD/cfff-01ff502a0784_HDD/public/qlz/projects/unified_gen/img_edit_pkl"
            filename = os.path.basename(debug_tgt_image.replace(".png", ".pkl"))
            pkl_data = {
                "siglip2_img_pooled_output":siglip2_img_pooled_output,
                "prompt":debug_prompt,
                "src_image":debug_src_image,
                "tgt_image":debug_tgt_image
            }
            with open(os.path.join(debugOutputDir, filename), mode='wb') as f:
                pickle.dump(pkl_data, f)
        self.additional_logs = {}
        for k in vila_vh_outputs.keys():
            self.additional_logs[k] = vila_vh_outputs[k].item() # transfer tensor to scalar
