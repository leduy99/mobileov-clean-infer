...
class SanaMSVideo(Sana):
    ...
    def forward(self, x, timestep, y, mask=None, **kwargs):
        ...
        if data_info.get("image_embeds_mask", None) is not None:
            image_mask = data_info["image_embeds_mask"].to(dtype=torch.long, device=x.device)
            if image_mask.dim() == 4:
                image_mask = image_mask.squeeze(1).squeeze(1)
            elif image_mask.dim() == 3:
                image_mask = image_mask.squeeze(1)
            elif image_mask.dim() == 1:
        ...
        if self.cross_attn_image_embeds:
            if mask is not None:
                ...
                y_lens = mask.squeeze(1).squeeze(1)
            else:
                y_lens = torch.ones((y.shape[0], y.shape[2]), device=y.device, dtype=torch.int16)
            y = y.squeeze(1)
