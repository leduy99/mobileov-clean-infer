"""
Mobile-OV Model with SANA-video: Unified architecture with SmolVLM2 for understanding and SANA-video for generation.

This model replaces the understanding module (VisionHead) with SmolVLM2-500M,
and replaces the generation module (WAN) with SANA-video.

Key changes from MobileOVModel (WAN version):
- Uses SANA-video instead of WAN for generation
- Requires adapter layer to convert between WAN-compatible interface and SANA interface
- SANA uses different tensor formats: [B, C, T, H, W] instead of List[Tensor]
"""
...
_sana_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "third_party", "sana"))
if _sana_root not in sys.path:
    sys.path.insert(0, _sana_root)

# SANA video model + config
...
class MobileOVModelSANA(nn.Module):
    """
    Mobile-OV Model with SANA-video: SmolVLM2 (understanding) + SANA-video (generation)
    
    This model integrates SmolVLM2-500M as the understanding module,
    replacing the original VisionHead. The generation module uses SANA-video instead of WAN.
    
    Key differences from MobileOVModel (WAN version):
    - Uses SANA-video instead of WAN for generation
    - SANA uses [B, C, T, H, W] tensor format instead of List[Tensor]
    - Requires adapter layer to convert interface between WAN-compatible and SANA
    - SANA forward: forward(x, timestep, y, mask=None, **kwargs)
    - WAN forward: forward(x, t, context, seq_len, context_lens=None, **kwargs)
    ...
    def __init__(
        self,
        wan_model_or_ckpt_dir: Union[Dict[str, Any], Any],
        ...
        Args:
            wan_model_or_ckpt_dir: Packed SANA model/config (internal use; use from_pretrained)
        ...
        # Handle SANA model initialization (packed in dict from from_pretrained)
        if isinstance(wan_model_or_ckpt_dir, dict) and "sana_model" in wan_model_or_ckpt_dir:
            self.sana_model = wan_model_or_ckpt_dir["sana_model"]
            self.sana_config = wan_model_or_ckpt_dir["sana_config"]
        else:
            raise ValueError("MobileOVModelSANA must be constructed via from_pretrained() with SANA config/checkpoint.")

        # Keep wan_model alias for compatibility with existing training code
        self.wan_model = self.sana_model
        self.max_context_len = max_context_len or getattr(self.sana_config.text_encoder, "model_max_length", 300)
        self.wan_model.text_len = self.max_context_len

        self.sana_caption_channels = getattr(self.sana_config.text_encoder, "caption_channels", 2304)
        self.sana_context_proj = nn.LazyLinear(self.sana_caption_channels, bias=False)
        self.sana_vae_downsample_rate = getattr(self.sana_config.vae, "vae_downsample_rate", 8)
        ...
        # Visual Context Adapter (not supported for SANA yet)
        if use_visual_context_adapter:
            logger.warning("VisualContextAdapter is not wired for SANA; disabling it for MobileOVModelSANA.")
    ...
    def from_pretrained(
        ...
        """
        Create MobileOVModelSANA from pretrained checkpoints.
        
        Args:
            sana_ckpt_dir: Directory containing SANA checkpoint files
            sana_config_path: Path to SANA yaml config
        ...
        Returns:
            MobileOVModelSANA: The initialized model
        """
        if sana_config_path is None:
            raise ValueError("sana_config_path must be provided for MobileOVModelSANA")
        if sana_ckpt_dir is None:
            raise ValueError("sana_ckpt_dir must be provided for MobileOVModelSANA")

        # Load SANA config via pyrallis (dataclass) for full defaults.
        try:
            import pyrallis
            with open(sana_config_path, "r") as f:
                sana_config = pyrallis.load(SanaVideoConfig, f)
        except Exception as exc:
            raise RuntimeError(f"Failed to load SANA config from {sana_config_path}: {exc}") from exc
        ...
        # Build SANA model
        latent_size = sana_config.model.image_size // sana_config.vae.vae_downsample_rate
        model_cfg = model_video_init_config(sana_config, latent_size=latent_size)
        model_cfg["type"] = sana_config.model.model
        sana_model = build_model(
            model_cfg,
            use_fp32_attention=getattr(sana_config.model, "fp32_attention", False),
        )
        sana_model.eval()

        # Resolve checkpoint path
        ckpt_path = sana_config.model.load_from
        ...
        device = torch.device(f"cuda:{device_id}" if torch.cuda.is_available() else "cpu")
        sana_model = sana_model.to(device).to(precision_dtype)

        model = cls(
            wan_model_or_ckpt_dir={"sana_model": sana_model, "sana_config": sana_config},
        ...
        return model.to(device, dtype=precision_dtype)
    ...
    def encode_prompts_with_smolvlm2(
        ...
        Args:
            ...
            input_ids: Pre-tokenized input IDs (optional, overrides prompts)
            attention_mask: Attention mask for input_ids (optional)
            pixel_values: Vision inputs from processor (optional, requires input_ids with image tokens)
            pixel_attention_mask: Attention mask for pixel_values (optional)
        ...
        # If input_ids are not provided, tokenize prompts
        if input_ids is None:
            if prompts is None:
                raise ValueError("prompts or input_ids must be provided for SmolVLM2 encoding")
        ...
        if pixel_values is not None:
            if input_ids is None:
                raise ValueError("pixel_values requires input_ids with image tokens (use a processor to build inputs)")
    ...
    def forward(
        ...
        smolvlm2_input_ids: Optional[torch.Tensor] = None,
        smolvlm2_attention_mask: Optional[torch.Tensor] = None,
        smolvlm2_pixel_values: Optional[torch.Tensor] = None,
        smolvlm2_pixel_attention_mask: Optional[torch.Tensor] = None,
        ...
        Args:
            x: Input tensor for SANA model (List[Tensor] or Tensor)
            t: Timestep tensor
            ...
            smolvlm2_input_ids: Pre-tokenized input IDs (optional, overrides prompts)
            smolvlm2_attention_mask: Attention mask for smolvlm2_input_ids (optional)
            smolvlm2_pixel_values: Vision inputs from processor (optional, requires smolvlm2_input_ids)
            smolvlm2_pixel_attention_mask: Pixel attention mask for smolvlm2_pixel_values (optional)
            visual_emb: Visual embeddings
            ref_images: Reference images
            seq_len: Sequence length (unused for SANA, kept for compatibility)
        ...
        if condition_mode == "full":
            ...
            # Only use ar_vision_input if it's already processed (aligned_emb shape [1152])
                ...
                # Use SmolVLM2 to encode prompts on-the-fly (optionally with vision inputs)
                if prompts is None and smolvlm2_input_ids is None:
                    raise ValueError("prompts or smolvlm2_input_ids must be provided when use_precomputed_features=False")
                ...
                if smolvlm2_input_ids is None:
                    ...
                    if any(use_uncond_mask):
                        logger.warning("CFG requested with smolvlm2_input_ids; provide unconditional inputs or disable CFG for multimodal runs.")

                # Encode prompts with SmolVLM2 (including null prompts for CFG samples)
                smolvlm2_hidden = self.encode_prompts_with_smolvlm2(
                    modified_prompts,
                    device,
                    input_ids=smolvlm2_input_ids,
                    attention_mask=smolvlm2_attention_mask,
                    pixel_values=smolvlm2_pixel_values,
                    pixel_attention_mask=smolvlm2_pixel_attention_mask,
                )  # [B, L, 1024]
        ...
        # Forward through SANA model
        if len(mixed_context) > 0:
            print(f"[DEBUG MobileOV] SANA context input shape: {mixed_context[0].shape}, context_lens: {context_lens.tolist()}", flush=True)
        import time as time_module
        sana_start = time_module.time()
        if torch.cuda.is_available():
            torch.cuda.synchronize()

        # Build padded context + mask for SANA
        context_lens_list = context_lens.tolist()
        max_len = max(context_lens_list) if context_lens_list else 0
        context_dim = mixed_context[0].shape[-1]
        context_batch = mixed_context[0].new_zeros((batch_size, max_len, context_dim))
        mask = torch.zeros((batch_size, max_len), device=context_batch.device, dtype=torch.long)
        for i, ctx in enumerate(mixed_context):
            length = int(context_lens_list[i])
            context_batch[i, :length] = ctx[:length]
            mask[i, :length] = 1

        # Project WAN-style context to SANA caption channels
        context_batch = self.sana_context_proj(context_batch)
        context_batch = context_batch[:, None, :, :]  # [B, 1, L, D]

        # Normalize input latents to tensor
        ...
        if not torch.is_tensor(timestep):
            timestep = torch.tensor(timestep, device=x_tensor.device)
        if timestep.dim() == 0:
            timestep = timestep.unsqueeze(0).expand(x_tensor.shape[0])
        elif timestep.dim() == 1 and timestep.shape[0] != x_tensor.shape[0]:
            timestep = timestep.expand(x_tensor.shape[0])

        height = x_tensor.shape[-2] * self.sana_vae_downsample_rate
        width = x_tensor.shape[-1] * self.sana_vae_downsample_rate
        hw = torch.tensor([[height, width]], dtype=torch.float, device=x_tensor.device).repeat(x_tensor.shape[0], 1)
        data_info = {"img_hw": hw}

        result = self.sana_model(
            x_tensor,
            timestep,
            context_batch,
            mask=mask,
        ...
        if torch.cuda.is_available():
            torch.cuda.synchronize()
        sana_time = time_module.time() - sana_start

        print(f"[PROFILE MobileOV] SANA forward pass: {sana_time*1000:.2f}ms", flush=True)
        ...
        logger.info(f"[PROFILE] SANA forward pass: {sana_time*1000:.2f}ms")
    ...
    def enable_gradient_checkpointing(self):
        ...
        if hasattr(self.wan_model, "enable_gradient_checkpointing"):
    ...
    def reset_wan_text_len(self, new_len: int):
        """Reset the cached max context length."""
        self.max_context_len = new_len
        if hasattr(self.wan_model, "text_len"):
    ...
    def save_pretrained(self, save_dir: str):
        ...
        # Save SANA model
        wan_save_dir = os.path.join(save_dir, "sana_model")
        os.makedirs(wan_save_dir, exist_ok=True)
        torch.save(self.wan_model.state_dict(), os.path.join(wan_save_dir, "pytorch_model.bin"))
