"""
Gemma 4 prompt bridge modules for bridge-only SANA experiments.

This module mirrors the trainer-facing interface of the existing Smol/Qwen
bridges (`smolvlm2_model`, `projector`, `forward`, etc.) while swapping the
frozen text backbone to Gemma 4.
"""
...
def _import_gemma4():
    try:
        ...
        raise RuntimeError(
            "Failed to import Gemma 4 dependencies. "
            "Use a Transformers build that supports Gemma 4, for example "
            "`pip install git+https://github.com/huggingface/transformers.git`."
        ) from exc
    return AutoTokenizer, Gemma4ForConditionalGeneration


class Gemma4SanaPromptBridge(nn.Module):
    """
    Gemma 4 text backbone -> bridge projector -> SANA prompt embeddings.

    This variant is text-only and intended for bridge-only experiments.
    """
    ...
    def __init__(
        ...
        if lora_enable:
            raise NotImplementedError("Gemma 4 bridge currently supports bridge-only runs only (lora_enable=false).")
        if use_vision_head:
            raise NotImplementedError("Gemma 4 bridge currently supports text-only mode only (use_vision_head=false).")
        ...
        self.tokenizer_model_id = str(tokenizer_model_id or gemma_ckpt_path)
        ...
        AutoTokenizer, Gemma4ForConditionalGeneration = _import_gemma4()
        self._tokenizer_cls = AutoTokenizer
        local_only = os.path.exists(gemma_ckpt_path)
        logger.info("Loading Gemma 4 backbone from %s (local_only=%s)", gemma_ckpt_path, local_only)
        self.gemma_model = Gemma4ForConditionalGeneration.from_pretrained(
            gemma_ckpt_path,
        ...
        logger.info("Loaded Gemma 4 backbone from %s", gemma_ckpt_path)
        self.gemma_model.eval().requires_grad_(False)
        ...
        self.smolvlm2_model = self.gemma_model

        gemma_hidden_size = None
        cfg = getattr(self.gemma_model, "config", None)
        if cfg is not None and hasattr(cfg, "text_config") and cfg.text_config is not None:
            gemma_hidden_size = getattr(cfg.text_config, "hidden_size", None)
        if gemma_hidden_size is None:
            gemma_hidden_size = getattr(cfg, "hidden_size", 1536)
        logger.info("Detected Gemma 4 hidden_size=%s", gemma_hidden_size)
        ...
        if self.projector_type in ("mcp_tiny", "mcp_full"):
            ...
            logger.info(
                "Using Gemma 4 MCP projector: type=%s d_vlm=%s d_cond=%s K=%s refine=%s",
                self.projector_type,
                gemma_hidden_size,
                caption_channels,
            ...
            if adapter_in_channels != gemma_hidden_size:
                logger.warning(
                    "Adapter in_channels (%s) != Gemma 4 hidden_size (%s); overriding adapter_in_channels.",
                    adapter_in_channels,
                    gemma_hidden_size,
                )
                adapter_in_channels = gemma_hidden_size
    ...
    def _get_tokenizer(self):
        if self._cached_tokenizer is None:
            self._cached_tokenizer = self._tokenizer_cls.from_pretrained(
                ...
                trust_remote_code=True,
                local_files_only=os.path.exists(self.tokenizer_model_id),
                padding_side="right",
            )
    ...
    def encode_prompts(
        ...
        if hasattr(outputs, "last_hidden_state"):
            ...
            raise RuntimeError("Gemma 4 output does not contain hidden states")
